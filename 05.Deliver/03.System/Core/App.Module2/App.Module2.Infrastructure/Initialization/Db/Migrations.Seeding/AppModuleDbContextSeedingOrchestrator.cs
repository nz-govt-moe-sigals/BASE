...
namespace App.Module02.Infrastructure.Db.Migrations.Seeding
{
    ...
    public class AppModuleDbContextSeedingOrchestrator: IHasModuleSpecificIdentifier
    {
        ...
        private void SeedByReflectionWontWorkFromPowershell(AppModuleDbContext context)
        {
        ...
        private void SeedByHand(AppModuleDbContext dbContext)
        {
            ...
            AppDependencyLocator.Current.GetInstance<AppModuleDbContextSeederBodyCategory>().Seed(dbContext);
            AppDependencyLocator.Current.GetInstance<AppModuleDbContextSeederBodyName>().Seed(dbContext);
            AppDependencyLocator.Current.GetInstance<AppModuleDbContextSeederBody>().Seed(dbContext);
            AppDependencyLocator.Current.GetInstance<AppModuleDbContextSeederBodyProperty>().Seed(dbContext);
            AppDependencyLocator.Current.GetInstance<AppModuleDbContextSeederBodyClaim>().Seed(dbContext);
            AppDependencyLocator.Current.GetInstance<AppModuleDbContextSeederEducationOrganisation>().Seed(dbContext);
        }
