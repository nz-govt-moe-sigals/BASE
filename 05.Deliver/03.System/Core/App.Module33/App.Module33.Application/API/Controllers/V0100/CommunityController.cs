...
{
    [AllowAnonymous]
    public class CommunityController : GuidIdActiveRecordStateODataControllerBase<Community, CommunityDto>
    {
        ...
        public IQueryable<CommunityDto> Get()
        {
            IQueryable<CommunityDto> results;

            try
            {
                results =
                    InternalActiveRecords()
                        .ProjectTo<CommunityDto>(
                            //Always either define null
                            //(object)null
                            // or include related tables:
                            //x => x.Category,
                            //x => x.Properties
                            x => x.CoherentPathways
                        );
            }
...
#pragma warning restore CS0168 // Variable is declared but never used
        ...
        //[ODataRoute("({key})")]
        public CommunityDto Get(Guid key)
        {
            var result =
                    InternalActiveRecords()
        ...
        //// POST api/values 
        public void Post(CommunityDto value)
        {
            InternalPost(value);
        }

        //// PUT api/values/5 
        public void Put(CommunityDto value)
        {
            InternalPut(value);
        }
    }
