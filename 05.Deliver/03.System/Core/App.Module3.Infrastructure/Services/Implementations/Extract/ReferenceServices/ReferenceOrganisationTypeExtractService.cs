...
namespace App.Module3.Infrastructure.Services.Implementations.Extract.ReferenceServices
{
    ...
    {
        public ReferenceOrganisationTypeExtractService(BaseExtractServiceConfiguration configuration, IExtractRepositoryService reposorityService, IExtractAzureDocumentDbService documentDbService)
            : base(configuration, reposorityService,  documentDbService)
        {

        }

        public override void UpdateLocalData(ReferenceOrganisationType item)
        {
            var mappedEntity = Mapper.Map<ReferenceOrganisationType, EducationProviderType>(item);
            var areaUnitsLookup = _repositoryService.GetSifCachedData< EducationProviderType>(); // is CACHED DATA
